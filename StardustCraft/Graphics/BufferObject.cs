...
namespace StardustCraft.Graphics
{
    public class BufferObject<T> : IGraphicsObject, IDisposable where T : unmanaged
    {
        ...
        public unsafe BufferObject(BufferTarget type, int size, bool isDynamic)
        {
            ...
            GL.BufferData(type, size * Marshal.SizeOf<T>(), IntPtr.Zero, isDynamic ? BufferUsageHint.StreamDraw : BufferUsageHint.StaticDraw);
        }
        ...
        public unsafe BufferObject(BufferTarget type, T[] data, bool isDynamic)
        {
            ...
            ID = GL.GenBuffer();
            GL.BindBuffer(type, ID);
            GL.BufferData(type, data.Length * Marshal.SizeOf<T>(), data, isDynamic ? BufferUsageHint.StreamDraw : BufferUsageHint.StaticDraw);
        }
